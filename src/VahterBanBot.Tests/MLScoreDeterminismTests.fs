...
type MLScoreDeterminismTests(fixture: MlEnabledVahterTestContainers, _ml: MlAwaitFixture) =
    ...
    [<InlineData("77",  0.25325489044189453)>]   // key boundary text: LlmTriageTests + MLBanTests depend on this being potential spam
    [<InlineData("33",  0.3628414571285248)>]    // used in MLBanTests for potential spam callbacks

    // -- Detected spam (score ≥ 1.0): auto-deleted, NotASpam callback only.
    [<InlineData("2222222",  1.8009693622589111)>]  // primary detected spam text in MLBanTests
    [<InlineData("7777777",  1.8824436664581299)>]  // detected spam, used for false-positive button tests
    [<InlineData("3333333",  1.8695933818817139)>]  // detected spam, false-negative scenario
    [<InlineData("66666666", 1.9190943241119385)>]  // detected spam, used in multiple MLBanTests

    // -- Texts scoring in unexpected ranges (but deterministic).
    [<InlineData("a",     0.037740349769592285)>]  // false-positive text; scores as potential spam because seed has both spam/ham 'a' entries
    [<InlineData("hello", 0.037740349769592285)>]  // natural language, not in seed — same score as "a" (model default for unknown alpha text)

    // -- Ham (score < 0.0): ignored by ML pipeline, no action taken.
    [<InlineData("b",     -0.3987234830856323)>]   // single letter ham, used in MLBanTests
    [<InlineData("1",     -0.33117640018463135)>]  // single digit from seed — short length keeps score below threshold
